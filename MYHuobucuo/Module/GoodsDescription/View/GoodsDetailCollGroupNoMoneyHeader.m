//
//  GoodsDetailCollGroupNoMoneyHeader.m
//  MYHuobucuo
//
...
@interface GoodsDetailCollGroupNoMoneyHeader () <UITableViewDataSource, UITableViewDelegate ,SDCycleScrollViewDelegate>
...
@implementation GoodsDetailCollGroupNoMoneyHeader
...
- (void)addGoodsNameView
{
    ...
    [groupHavePersonLabel setFont:[UIFont systemFontOfSize:fScreen(24)]];
    ...
    [groupHavePersonLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        CGSize textSize = [groupHasNumberString sizeForFontsize:fScreen(24)];
        make.width.mas_equalTo(textSize.width + 2);
    }];
    
    // 团人数
    UILabel *groupCountLabel = [[UILabel alloc] init];
    [groupCountLabel setFont:[UIFont systemFontOfSize:fScreen(20)]];
    [groupCountLabel setBackgroundColor:HexColor(0xe44a62)];
    [groupCountLabel setTextColor:[UIColor whiteColor]];
    [groupCountLabel setTextAlignment:NSTextAlignmentCenter];
    [groupCountLabel.layer setCornerRadius:4.f];
    [groupCountLabel.layer setMasksToBounds:YES];
    NSString *groupCountString = [NSString stringWithFormat:@"%ld人团", self.goodsModel.groupNumber];
    [groupCountLabel setText:groupCountString];
    [view addSubview:groupCountLabel];
    [groupCountLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(groupHavePersonLabel.mas_left).offset(-fScreen(12));
        make.height.mas_equalTo(fScreen(32));
        make.width.mas_equalTo(fScreen(75));
        make.centerY.equalTo(groupHavePersonLabel);
    }];
    ...
    [marketPriceLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(groupCountLabel.mas_left).offset(-fScreen(20));
    }];
    ...
    [goodsInfoLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(view).offset(fScreen(28));
        make.right.equalTo(view).offset(-fScreen(28));
        make.top.equalTo(priceLabel.mas_bottom).offset(fScreen(18));
